...
public:
	...
	void configure() {
		...
		m_worldToGrid =
			Transform::scale(
#if 0
				Vector((m_res[0] - 1)/extents[0], (m_res[1] - 1)/extents[1], (m_res[2] - 1)/extents[2])
#else
				Vector(m_res[0]/extents[0], m_res[1]/extents[1], m_res[2]/extents[2])
#endif
			)
...
#else
			m_stepSize = 0.5f * std::min(m_stepSize, extents[i] / (Float) m_res[i]);
#endif
		m_aabb.reset();
		for (int i=0; i<8; ++i)
...
#else
        if ( x1 < 0 || x1 >= m_res.x || y1 < 0 || y1 >= m_res.y || z1 < 0 || z1 >= m_res.z ) return 0;
        const int ix = x1, iy = y1, iz = z1;
#endif
	...
	Spectrum lookupSpectrum(const Point &_p) const {
        ...
        const int iz = fz < 0.5f ? z1 : z2;
#else
        if ( x1 < 0 || x1 >= m_res.x || y1 < 0 || y1 >= m_res.y || z1 < 0 || z1 >= m_res.z ) return Spectrum(0.0f);
        const int ix = x1, iy = y1, iz = z1;
#endif
	...
	Vector lookupVector(const Point &_p) const {
        ...
        const int iz = fz < 0.5f ? z1 : z2;
#else
        if ( x1 < 0 || x1 >= m_res.x || y1 < 0 || y1 >= m_res.y || z1 < 0 || z1 >= m_res.z ) return Vector(0.0f);
        const int ix = x1, iy = y1, iz = z1;
#endif
