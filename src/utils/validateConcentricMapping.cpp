...
class ValidateConcentricMapping : public Utility {
public:
...
// 			tmp[0], tmp[1], tmp[2], bRec.wo.x, bRec.wo.y, bRec.wo.z);
		...
		calcEvalBSDF(baseBSDF, m_wi, samples, bsdfValues);
		outputBitmap(bsdfValues, "bsdf_eval.exr");
 
		calcSampleBSDF(baseBSDF, m_wi, bsdfValues);
		outputBitmap(bsdfValues, "bsdf_sample.exr");
...
#pragma omp parallel for
			...
			for (int j = 0; j < m_size; j++) {
				...
				for (int x = 0; x < m_sqrtSpp; x++) {
					for (int y = 0; y < m_sqrtSpp; y++) {
						double dx = (j + samples[x][y].x) / (double)m_size;
						double dy = (i + samples[x][y].y) / (double)m_size;
						
						Vector wo = warp::squareToUniformHemisphereConcentric(Point2(dx, dy));
	...
	void calcSampleBSDF(BSDF *bsdf, const Vector &wi, Vector2DArray &bsdfValues) {
		...
		Float normFactor = (double)m_size * m_size / (2.0 * M_PI) / spp;
...
MTS_EXPORT_UTILITY(ValidateConcentricMapping, "Validate concentric mapping")
MTS_NAMESPACE_END