/*
	...
	keep tracking of derivatives with respect to albedo and lobe scales
*/
...
* }
*/
class SimpleMultiLobeDiffVolumetricPathTracer : public MonteCarloIntegrator {
public:
	SimpleMultiLobeDiffVolumetricPathTracer(const Properties &props) : MonteCarloIntegrator(props) {
	...
	SimpleMultiLobeDiffVolumetricPathTracer(Stream *stream, InstanceManager *manager)
	...
	void configure() {
		...
		LdW.resize(m_numLobes);
		for (int i = 0; i < m_numLobes; i++) {
			LdW[i] = new Spectrum[pixelNum];
			for (int p = 0; p < pixelNum; p++)
				LdW[i][p] = Spectrum(0.f);
		}
	}

	~SimpleMultiLobeDiffVolumetricPathTracer() {
		if (LdA != NULL)
			delete[] LdA;
		//if (TdA != NULL)
		//	delete[] TdA;
	...
	Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec, 
		Spectrum &TdA, Spectrum &LdA, std::vector<Spectrum> &TdW, std::vector<Spectrum> &LdW, 
		int &albedoSegs) const {
				...
				*/
				...
				if (rRec.type & RadianceQueryRecord::EDirectMediumRadiance) {
					...
					if (!value.isZero()) {
						...
						for (int k = 0; k < m_numLobes; k++) {
							LdW[k] += TdW[k] * val + throughput * value * weightedF[k];
						}
					}
				}
	...
	int m_numLobes;
	std::vector<Spectrum*> LdW;

	MTS_DECLARE_CLASS()
};

MTS_IMPLEMENT_CLASS_S(SimpleMultiLobeDiffVolumetricPathTracer, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(SimpleMultiLobeDiffVolumetricPathTracer, "Simple volumetric path tracer with albedo and lobe scales diff");
MTS_NAMESPACE_END
