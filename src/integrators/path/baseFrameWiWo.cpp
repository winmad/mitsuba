 ...
 * }
 */
class BaseFrameWiWoPathTracer : public MonteCarloIntegrator {
public:
	BaseFrameWiWoPathTracer(const Properties &props)
	...
	BaseFrameWiWoPathTracer(Stream *stream, InstanceManager *manager)
	...
	void configure() {
		m_wi = new Bitmap(Bitmap::ERGB, Bitmap::EFloat32, Vector2i(m_width, m_height));
		m_wo = new Bitmap(Bitmap::ERGB, Bitmap::EFloat32, Vector2i(m_width, m_height));
	}
	...
	void renderBlockX(const Scene *scene,
		...
		const bool &stop, const std::vector< TPoint2<uint8_t> > &points) {
			...
			float *wiData = m_wi->getFloat32Data();
			float *woData = m_wo->getFloat32Data();
			...
			for (size_t i = 0; i < points.size(); ++i) {
				...
				// Actually, wi is the view direction, and wo is the light direction
				Vector avgWi(0.0);
				Vector avgWo(0.0);
				Float numValidWi = 0.0;
				Float numValidWo = 0.0;
				...
				for (size_t j = 0; j < sampler->getSampleCount(); j++) {
					...
					Vector wiMacro, woMacro;
					bool validWi = false, validWo = false;
					spec *= Li(sensorRay, rRec, wiMacro, validWi, woMacro, validWo);
	...
	Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec, 
			Vector &wiMacro, bool &validWi, Vector &woMacro, bool &validWo) const {
	...
	std::string toString() const {
		...
		oss << "BaseFrameWiWoTracer[" << endl
			<< "  maxDepth = " << m_maxDepth << "," << endl
			<< "  rrDepth = " << m_rrDepth << "," << endl
			<< "  strictNormals = " << m_strictNormals << "," << endl
			<< "  queryDepth = " << m_queryDepth << endl
			<< "]";
		return oss.str();
	}
...
MTS_EXPORT_PLUGIN(BaseFrameWiWoPathTracer, "Wi or Wo in baseFrame path tracer");
MTS_NAMESPACE_END
