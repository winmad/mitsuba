...
* }
*/
class SimpleScaleDiffVolumetricPathTracer : public MonteCarloIntegrator {
public:
	SimpleScaleDiffVolumetricPathTracer(const Properties &props) : MonteCarloIntegrator(props) {
	...
	SimpleScaleDiffVolumetricPathTracer(Stream *stream, InstanceManager *manager)
	...
	~SimpleScaleDiffVolumetricPathTracer() {
		for (int i = 0; i < numClusters; i++) {
			if (LdA[i] != NULL)
				delete[] LdA[i];
			if (TdA[i] != NULL)
...
MTS_IMPLEMENT_CLASS_S(SimpleScaleDiffVolumetricPathTracer, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(SimpleScaleDiffVolumetricPathTracer, "Simple volumetric path tracer with albedo scale diff");
MTS_NAMESPACE_END
