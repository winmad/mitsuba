...
//#define COMPOSITE_HEIGHTFIELD_DEBUG
...
public:
	CompositeHeightfield(const Properties &props) : Shape(props), m_data(NULL), m_normals(NULL), m_minmax(NULL),
			m_baseData(NULL), m_baseMinmax(NULL) {
		m_sizeHint = Vector2i(
			props.getInteger("width", -1),
			props.getInteger("height", -1)
		);

		m_inputObjectToWorld = props.getTransform("toWorld", Transform());
		...
		m_baseFilename = props.getString("baseFilename", "");
		m_baseScale = props.getFloat("baseScale", 1);
	}

	CompositeHeightfield(Stream *stream, InstanceManager *manager) : Shape(stream, manager) {
		NotImplementedError("CompositeHeightfield(Stream, Manager)");
	}
	...
	~CompositeHeightfield() {
		...
		if (m_baseMinmax) {
			for (int i = 0; i < m_baseLevelCount; ++i)
				freeAligned(m_baseMinmax[i]);
			delete[] m_baseMinmax;
			delete[] m_baseLevelSize;
			delete[] m_baseNumChildren;
			delete[] m_baseBlockSize;
			delete[] m_baseBlockSizeF;
		}
		if (m_normals)
	...
	void serialize(Stream *stream, InstanceManager *manager) const {
		NotImplementedError("CompositeHeightfield::serialization");
	}
	...
	inline void getBaseHeightsAtPatchCorners(Float blockX, Float blockY, int x, int y,
		...
		for (int dv = 0; dv < 2; dv++) {
			Float vCur = v + stepV * dv;
			Float wv = std::abs(1.0 - (vCur * m_baseDataSize.y - yBase));
			for (int du = 0; du < 2; du++) {
				Float uCur = u + stepU * du;
				Float wu = std::abs(1.0 - (uCur * m_baseDataSize.x - xBase));
				res[dv][du] = cornerVals[0][0] * wv * wu +
					cornerVals[0][1] * wv * (1.0 - wu) +
					cornerVals[1][0] * (1.0 - wv) * wu +
					cornerVals[1][1] * (1.0 - wv) * (1.0 - wu);
			}
		}
	...
	void fillIntersectionRecord(const Ray &ray,
		...
		if (m_shadingNormals) {
			Log(EError, "Don't support shading normals");
		}
		else {
	...
	void getNormalDerivative(const Intersection &its,
		...
		NotImplementedError("CompositeHeightfield::getNormalDerivative");		
	}
	...
	size_t configureTiledHeightfield() {
		// load tiled height map
		if (!m_filename.empty()) {
			if (m_bitmap.get())
		...
		// configure minmax of tiled height map
		...
		m_objectToWorld = m_inputObjectToWorld * Transform::translate(Vector(-1, -1, 0)) *
	...
	size_t configureBaseHeightfield() {
		// load base height map
		if (!m_baseFilename.empty()) {
			if (m_baseBitmap.get())
				Log(EError, "Cannot specify a file name and a nested texture at the same time!");
			ref<FileStream> fs = new FileStream(m_baseFilename, FileStream::EReadOnly);
			m_baseBitmap = new Bitmap(Bitmap::EAuto, fs);
		}
		else if (!m_baseBitmap.get()) {
			Log(EError, "A height field texture must be specified (either as a nested texture, or using the 'baseFilename' parameter)");
		}

		m_baseDataSize = m_baseBitmap->getSize();
		if (m_baseDataSize.x < 2) m_baseDataSize.x = 2;
		if (m_baseDataSize.y < 2) m_baseDataSize.y = 2;
		if (!math::isPowerOfTwo(m_baseDataSize.x)) m_baseDataSize.x = (int)math::roundToPowerOfTwo((uint32_t)m_baseDataSize.x);
		if (!math::isPowerOfTwo(m_baseDataSize.y)) m_baseDataSize.y = (int)math::roundToPowerOfTwo((uint32_t)m_baseDataSize.y);

		if (m_baseBitmap->getSize() != m_baseDataSize) {
			m_baseBitmap = m_baseBitmap->convert(Bitmap::ELuminance, Bitmap::EFloat);

			Log(EInfo, "Resampling heightfield texture from %ix%i to %ix%i ..",
				m_baseBitmap->getWidth(), m_baseBitmap->getHeight(), m_baseDataSize.x, m_baseDataSize.y);

			m_baseBitmap = m_baseBitmap->resample(m_rfilter, ReconstructionFilter::EClamp,
				ReconstructionFilter::EClamp, m_baseDataSize,
				-std::numeric_limits<Float>::infinity(),
				std::numeric_limits<Float>::infinity());
		}

		/*
		int channelCount = m_baseBitmap->getChannelCount();
		Log(EInfo, "bitmap size = (%d, %d), channel_count = %d", m_baseBitmap->getSize().x, m_baseBitmap->getSize().y, channelCount);
		Log(EInfo, "pixel format = %d", (int)m_baseBitmap->getPixelFormat());
		Log(EInfo, "component format = %d", (int)m_baseBitmap->getComponentFormat());
		Log(EInfo, "bytes per pixel = %d", m_baseBitmap->getBytesPerPixel());
		Log(EInfo, "buffer size = %d", m_baseBitmap->getBufferSize());

		float *tmpData = m_baseBitmap->getFloat32Data();
		Log(EInfo, "%.6f", tmpData[0]);
		*/

		// configure minmax of base height map
		size_t size = (size_t)m_baseDataSize.x * (size_t)m_baseDataSize.y * sizeof(Float);
		m_baseData = (Float *)allocAligned(size);
		m_baseBitmap->convert(m_baseData, Bitmap::ELuminance, Bitmap::EFloat, 1.0f, m_baseScale);

		m_baseBitmap = NULL;

		m_baseToWorld = m_inputObjectToWorld * Transform::translate(Vector(-1, -1, 0)) *
			Transform::scale(Vector((Float)2 / m_baseDataSize.x, (Float)2 / m_baseDataSize.y, 1));
		m_worldToBase = m_baseToWorld.inverse();

		size_t storageSize = (size_t)m_baseDataSize.x * (size_t)m_baseDataSize.y * sizeof(Float);
		Log(EInfo, "Building acceleration data structure for %ix%i height field ..", m_baseDataSize.x, m_baseDataSize.y);

		m_baseLevelCount = (int)std::max(math::log2i((uint32_t)m_baseDataSize.x), math::log2i((uint32_t)m_baseDataSize.y)) + 1;
		
		m_baseLevelSize = new Vector2i[m_baseLevelCount];
		m_baseNumChildren = new Vector2i[m_baseLevelCount];
		m_baseBlockSize = new Vector2i[m_baseLevelCount];
		m_baseBlockSizeF = new Vector2[m_baseLevelCount];
		m_baseMinmax = new Interval*[m_baseLevelCount];

		m_baseLevelSize[0] = Vector2i(m_baseDataSize.x, m_baseDataSize.y);
		m_baseLevelSize0f = Vector2(m_baseLevelSize[0]);
		m_baseBlockSize[0] = Vector2i(1, 1);
		m_baseBlockSizeF[0] = Vector2(1, 1);
		m_invBaseSize = Vector2((Float)1 / m_baseLevelSize[0].x, (Float)1 / m_baseLevelSize[0].y);
		
		size = (size_t)m_baseLevelSize[0].x * (size_t)m_baseLevelSize[0].y * sizeof(Interval);
		m_baseMinmax[0] = (Interval *)allocAligned(size);
		storageSize += size;

		Interval *bounds = m_baseMinmax[0];
		for (int y = 0; y < m_baseLevelSize[0].y; ++y) {
			for (int x = 0; x < m_baseLevelSize[0].x; ++x) {
				Float f00 = m_baseData[y * m_baseDataSize.x + x];
				Float f10 = m_baseData[y * m_baseDataSize.x + (x + 1) % m_baseDataSize.x];
				Float f01 = m_baseData[((y + 1) % m_baseDataSize.y) * m_baseDataSize.x + x];
				Float f11 = m_baseData[((y + 1) % m_baseDataSize.y) * m_baseDataSize.x + (x + 1) % m_baseDataSize.x];
				Float fmin = std::min(std::min(f00, f01), std::min(f10, f11));
				Float fmax = std::max(std::max(f00, f01), std::max(f10, f11));
				*bounds++ = Interval(fmin, fmax);
			}
		...
		for (int level = 1; level < m_baseLevelCount; ++level) {
			Vector2i &cur = m_baseLevelSize[level],
				&prev = m_baseLevelSize[level - 1];

			cur.x = prev.x > 1 ? (prev.x / 2) : 1;
			cur.y = prev.y > 1 ? (prev.y / 2) : 1;

			m_baseNumChildren[level].x = prev.x > 1 ? 2 : 1;
			m_baseNumChildren[level].y = prev.y > 1 ? 2 : 1;
			m_baseBlockSize[level] = Vector2i(
				m_baseLevelSize[0].x / cur.x,
				m_baseLevelSize[0].y / cur.y
			);
			m_baseBlockSizeF[level] = Vector2(m_baseBlockSize[level]);

			Interval *prevBounds = m_baseMinmax[level - 1], *curBounds;
			size_t size = (size_t)cur.x * (size_t)cur.y * sizeof(Interval);
			m_baseMinmax[level] = curBounds = (Interval *)allocAligned(size);
			storageSize += size;
	...
	void configure() {
		...
		size_t storageSize = configureTiledHeightfield();
		storageSize += configureBaseHeightfield();
		...
		m_dataAABB = AABB(
			Point3(0, 0, m_minmax[m_levelCount - 1][0].min + m_baseMinmax[m_baseLevelCount - 1][0].min),
			Point3(m_levelSize0f.x, m_levelSize0f.y, m_minmax[m_levelCount - 1][0].max + m_baseMinmax[m_baseLevelCount - 1][0].max)
		);
	...
	std::string toString() const {
		...
		oss << "CompositeHeightfield[" << endl
			<< " Tiled heightmap:" << endl
			<< "  size = " << m_dataSize.toString() << "," << endl
			<< "  tileX = " << m_tileX << "," << endl
			<< "  tileY = " << m_tileY << "," << endl
			<< "  minH = " << m_minmax[m_levelCount - 1][0].min << "," << endl
			<< "  maxH = " << m_minmax[m_levelCount - 1][0].max << "," << endl
			<< " Base heightmap:" << endl
			<< "  size = " << m_baseDataSize.toString() << "," << endl
			<< "  minH = " << m_baseMinmax[m_baseLevelCount - 1][0].min << "," << endl
			<< "  maxH = " << m_baseMinmax[m_baseLevelCount - 1][0].max << "," << endl
			<< "  shadingNormals = " << m_shadingNormals << "," << endl
			<< "  flipNormals = " << m_flipNormals << "," << endl
			<< "  objectToWorld = " << indent(m_objectToWorld.toString()) << "," << endl
			<< "  baseToWorld = " << indent(m_baseToWorld.toString()) << "," << endl
...
private:
	...
	Transform m_inputObjectToWorld;
	Transform m_objectToWorld, m_worldToObject;
	Vector2i m_sizeHint;
	AABB m_dataAABB;
	bool m_shadingNormals;
	bool m_flipNormals;
	fs::path m_filename, m_baseFilename;

	/* Tiled (noise) height field data */
	ref<Bitmap> m_bitmap;
	Float m_scale;
	Float *m_data;
	Normal *m_normals;
	...
	/* Base height field data */
	ref<Bitmap> m_baseBitmap;
	Float m_baseScale;
	Float *m_baseData;
	Vector2i m_baseDataSize;
	Vector2 m_invBaseSize;
	Transform m_baseToWorld, m_worldToBase;

	/* Min-max quadtree data for base */
	int m_baseLevelCount;
	Vector2i *m_baseLevelSize;
	Vector2   m_baseLevelSize0f;
	Vector2i *m_baseNumChildren;
	Vector2i *m_baseBlockSize;
	Vector2  *m_baseBlockSizeF;
	Interval **m_baseMinmax;
...
MTS_IMPLEMENT_CLASS_S(CompositeHeightfield, false, Shape)
MTS_EXPORT_PLUGIN(CompositeHeightfield, "Compositing two heightfields");
MTS_NAMESPACE_END
